...
VERSION = 2  #12/18/2024 - New way of handling pens in settings.json
#Pen 1 & 2 share the same reader but different antenna 
divider = "\n"+"-"*40+"\n"

VERSION_MESSAGE = divider
VERSION_MESSAGE += "Version:"+ str(VERSION)
VERSION_MESSAGE += "\nUpdated settings.json to settings-new.json to handle one reader and multiple pens"
VERSION_MESSAGE += divider 
...
class RFIDReaderApp(Frame):
    ...
    def __init__(self, master):
       ...
       ###### Version Info Tab
        ...
        #self.pauseComm=True
        try:
            self.json_file = "settings-new.json"
    ...
    def startMainRoutine(self):     
        ...
        for (asr,camera,penNo,readerNo,donotcare) in self.standalone_readers:
            ...
            if(not quickUpdate):   
                ...
                for pen in json_obj["pens"]:
                    ...
                    for reader in readers:
                        if reader["camera"] != "0":
                            #V2-12/18/2024 - check if split_readers is a key
                            if("split_antennas" in reader):
                                #antenna_breakdown = [{"pen":1,"antenna":[1,2]},{"pen":2,"antenna":[3,4]}]
                                antenna_breakdown = reader["split_antennas"]
                                self.asr_camera_pairs.append((reader["ip"],reader["camera"], penNo, reader["no"],antenna_breakdown))
                            else:
                                self.asr_camera_pairs.append((reader["ip"],reader["camera"], penNo, reader["no"],-1))  #-1 means no split antenna

                        else:
                            if("split_antennas" in reader):
                                #antenna_breakdown = [{"pen":1,"antenna":[1,2]},{"pen":2,"antenna":[3,4]}]
                                antenna_breakdown = reader["split_antennas"]
                                self.standalone_readers.append([reader["ip"], reader["camera"], penNo, reader["no"],antenna_breakdown]) 
                            else:
                                self.standalone_readers.append([reader["ip"], reader["camera"], penNo, reader["no"],-1]) #-1 means no split antenna
    ...
    def infoIP(self,address):
        ...
        if(len(self.asr_camera_pairs) > 0):
            for pair in self.asr_camera_pairs:
                if(ip==pair[0]):
                    return ("asr",ip,port,pair[2],pair[3],pair[4])
                elif(ip==pair[1]):
                    return ("camera",ip,port,pair[2],"n/a",pair[4])        

        if(len(self.standalone_readers) > 0):
            for asr in self.standalone_readers:
                if(ip==asr[0]):
                    return ("asr",ip,port,asr[2],asr[3],asr[4])
    ...
    def threaded_client(self,connection,address):
        ...
        (type,ip,port,pen,reader,split_reader)=self.infoIP(address)  #this is the real/simulated asr responding to multi-id trigger request
        print(f"[SPLIT SPLIT SPLIT STATUS]==> {split_reader}") #V2-12/19/2024 - split status -1 means not reader is not split between pens else
        antenna_pen_mapping={}
        for i in range(8):
            antenna_pen_mapping[i+1]=pen  #if default, make all antennas 1-8 map to pen#N

        #if reader is not meant for splitting
        if(split_reader == -1):
            print(f"reader {reader} is not of the split type")
            antenna_pen_mapping
        else:
            #Else if reader is meant for antenna splits
            #split_reader will look something like [['127.0.0.1', '0', '1/2', '1A', [{'pen':1,'antenna':[2,3]},{'pen':2,"antenna":[4,5,6,7]}]]]
            #self.standalone_readers[0][4][0]["antenna"]
                       
            for pen_antenna in split_reader:
            
                antennas = pen_antenna['antenna']   #a list of antennas
                corresponding_pen = pen_antenna['pen'] #pen corresponding to antenna
                #for each antenna, update correponding pen (if needed) (by default maps to a single pen provided in json file)
                for antenna in antennas:
                    antenna_pen_mapping[antenna]=corresponding_pen
        #~~~~~~~~end of handling antenna splits  ~~~~~~~~~~~~~~~~      
                    ...
                    if not data: #client has disconnected
                        ...
                        if(len(reads)>0):
                            ...
                            for rfid in uniq_rfids:
                                ...
                                #For publishing
                                antenna = matches[0][1]
                                mapped_pen = antenna_pen_mapping[int(antenna)]
                                pub_dict = {"rfid":matches[0][0], "antennaNo": antenna,"pen":mapped_pen}
                            ...
                            if(len(final_reads) > 0):
                                #12/29/2024 - cannot replace pen by mapped_pen as the whole set of reads will have the same identifier
                                #in this case pen might be something like 1/2 and have reads from all 4 antennas
                                ...
                                self.sendToQueueMSU(now,pen,reader,final_reads,identifier,antenna_pen_mapping)
    ...
    #FOR MSU - adding antenn_pen_mapping so that the reads from antennas 1,2 map to pen A and 3,4 map to pen 2 or something like that
    def sendToQueueMSU(self, now,pen,reader,final_reads,identifier,antenna_pen_mapping):
        ...
        for read in final_reads:
            ...
            self.printToTerminal(f"Adding unique read  {identifier}---{mapped_pen}-{reader}-{eid}-{antenna}-{now} to queue") 
            #self.update_queue(pen,reader,eid,antenna,now) #update the deque

            #MSU - pen will be extracted from the antenna_pen_mapping
            #self.update_queue(pen,reader,eid,antenna,now,identifier) #update the deque
          
            self.update_queue(mapped_pen,reader,eid,antenna,now,identifier) #update the deque
            
            #msg = msg+f"RECEIVED - {pen}-{reader}-|| {eid}  @ {antenna}||  @ {now}\n"
            msg = msg+f"RECEIVED - {mapped_pen}-{reader}-|| {eid}  @ {antenna}||  @ {now}\n"
            #self.prnt(msg)
