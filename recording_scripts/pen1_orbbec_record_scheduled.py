...
class SceneBasedRepublisherAndRecorder(Node):
    def __init__(self, topic_names, message_types, rate_hz, max_duration_s, base_bag_dir="/workspaces/isaac_ros-dev/bags/pen1/3D", log_dir="/workspaces/isaac_ros-dev/recording_logs/pen1/3D"):
        ...
        while rclpy.ok():
            with self.lock:
                if not self.bag_process:
                    ...
                    topics_list = [f"/pen1{topic}" for topic in self.topic_names]

                    # Adding camera info of pen1
                    camera_infos = ["/pen1_rfid", "/camera_01/color/camera_info", "/camera_01/depth/camera_info", "/camera_02/color/camera_info", "/camera_02/depth/camera_info"]
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Directory for storing the bag file (default is /workspaces/isaac_ros-dev/bags/pen1/3D)."
    )
